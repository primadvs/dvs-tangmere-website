...
- A Storage Account (`tangmerebackendstorage`) created and connected — the Function App creation wizard skipped this step when the hosting plan was switched partway through, which caused the first deployment to be "unhealthy." Fixed by creating it manually and adding its connection string as two application settings, `AzureWebJobsStorage` and `TABLES_CONNECTION_STRING`.
- `ALLOWED_ORIGINS` application setting added, so only requests from Tangmere's own site addresses are accepted.
...
- **A security note from today, already handled but worth recording:** a Storage Account access key was briefly pasted into this chat while debugging. It was rotated immediately and the new key is what's in use now — nothing further to do, but a reminder to always copy secrets directly from Azure into the relevant field, never through chat.
